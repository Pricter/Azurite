/**
 * <h1>This is a particle class (a single particle), a class that stores the position, magnitude, angle, mass of a single particle.</h1>
 *
...
public class Particle {
    ...
    public void update(float delta) {
        if (position.x < 0 || position.x > Window.getWidth()) {
            magnitude *= -1;
        } else if (position.y < 0 || position.y > Window.getHeight()) {
            magnitude *= -1;
        }
